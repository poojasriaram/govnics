...
export default function ESGPage() {
  const [selectedItem, setSelectedItem] = useState<EsgItem | null>(null);
  ...
  const renderCard = (item: EsgItem, idx: number) => (
    <div 
      key={idx} 
      onClick={() => setSelectedItem(item)}
      className="group h-full relative bg-white border border-slate-200/80 hover:border-blue-500/30 rounded-3xl transition-all duration-300 flex flex-col justify-between overflow-hidden shadow-md hover:shadow-xl p-6 cursor-pointer select-none min-h-[295px]"
    >
      ...
      <div className="pt-4 border-t border-slate-100 flex items-center justify-between relative z-10">
        ...
        <span className="flex items-center gap-0.5 text-[9px] font-bold text-blue-600 uppercase tracking-wider group-hover:underline">
          View Scope <ArrowRight className="w-3 h-3 text-blue-500 group-hover:translate-x-0.5 transition-transform" />
        </span>
      </div>
    </div>
  );

  const ehsCards = ehsItems.map((item, idx) => renderCard(item, idx));
  const laborCards = laborItems.map((item, idx) => renderCard(item, idx));
  const disclosureCards = disclosureItems.map((item, idx) => renderCard(item, idx));
  const climateCards = climateItems.map((item, idx) => renderCard(item, idx));
  const advisoryCards = advisoryItems.map((item, idx) => renderCard(item, idx));
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl border border-slate-200 max-w-2xl w-full p-8 shadow-2xl relative animate-slide-up-dropdown text-left space-y-6">
            ...
            <div className="space-y-2 pt-4 border-t border-slate-100">
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Key Project Deliverables</h4>
